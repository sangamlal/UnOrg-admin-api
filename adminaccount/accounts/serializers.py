...
class SignupUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=User
        fields=('username', 'password', 'confirm_password', 'email', 'first_name', 'last_name','mobile')
        # extra_kwargs={"confirm_password":{'write_only':True}}
    def validate(self, attrs):
        if attrs['password'] != attrs['confirm_password']:
            raise serializers.ValidationError({'password':"password and confirm_password did'nt match."})
        return attrs
...
class EditUserProfileSerializer(serializers.Serializer):
    ...
    email=serializers.EmailField(required=False,allow_blank=True,allow_null=True,validators=[UniqueValidator(queryset=User.objects.all())])
    username=serializers.CharField(required=False,allow_blank=True,allow_null=True,validators=[UniqueValidator(queryset=User.objects.all())])
